...
int main()
{
    ...
    Actor* camActor = gameEngine->GetWorld()->SpawnActor("Camera");
    camActor->AddComponent<CameraComponent>();
    camActor->GetTransform().SetWorldPosition(glm::vec3(0.0f, 2.0f, 6.0f));

    Actor* skybox = gameEngine->GetWorld()->SpawnActor("Skybox");
    ...
    Actor* actor1 = gameEngine->GetWorld()->SpawnActor("Cow");
    ...
    Actor* guiActor = gameEngine->GetWorld()->SpawnActor("GUI");
